...
def file2sql(json_files):
    ...
    for json_file in json_files:
        ...
        sql_list = [insert_rainfall, insert_water, insert_spec]
        ...
        for data_one in data:
            ...
            if device_id in (devices[0], devices[1]):
                rainfall_data.append((device_id, device_time, 'ADD', float(device_data[0]['data']), 50, update_time))
                # 构造含水率数据
                dp8 = float(device_data[1]['data']) if device_data[1]['data'] else 0.0
                dp1 = float(device_data[2]['data']) if len(device_data)>2 else 0.0
                spec_data.append((device_id, device_time, dp1, dp8))
            # 构造流速数据
            if device_id == devices[2]:
                water_id = hash_code(hash_key, device_id+str(device_time))
                water_data.append((water_id, device_id, 0.0, device_data[0]['data'], device_time, 0, update_time))
        
        # 合并数据
        sql_data = [tuple(rainfall_data), tuple(water_data), tuple(spec_data)]
...
def json2sql(json_data):
    ...
    # 构造降雨量数据
    if device_id in (devices[0], devices[1]):
        insert_data = insert_rainfall % (device_id, device_time, 'ADD', float(device_data[0]['data']), 50, update_time)
        # 构造含水率数据
        dp8 = float(device_data[1]['data']) if device_data[1]['data'] else 0.0
        dp1 = float(device_data[2]['data']) if len(device_data)>2 else 0.0
        insert_data = insert_spec % (device_id, device_time, dp1, dp8)
    # 构造流速数据
    if device_id == devices[2]:
        water_id = hash_code(hash_key, device_id+str(device_time))
        insert_data = insert_water % (water_id, device_id, 0.0, device_data[0]['data'], device_time, 0, update_time)
